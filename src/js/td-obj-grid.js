...
_TD.a.push(function (TD) {
	...
	var grid_obj = {
		 ...
		 */
		checkBlock: function () {
			// Panel map不需要寻路验证（只是UI展示区域）
			if (!this.map.is_main_map || !this.map.entrance || !this.map.exit) {
				return false;
			}
		...
		addBuilding: function (building_type, entityId) {
			...
			if (typeof TD._building_id_counter === "undefined") {
				TD._building_id_counter = 0;
			}
			var buildingDomId = "building-" + building_type + "-" + (++TD._building_id_counter);

			var building = new TD.Building(buildingDomId, {
				type: building_type,
				entityId: entityId,
		...
		onOut: function () {
			// 如果当前气球提示指向本格子，将其隐藏
			if (this.scene.panel && this.scene.panel.balloontip.el == this) {
		...
		onClick: function () {
			...
			if (TD.mode == "build" && this.map.is_main_map && !this.building) {
				...
				// 通过ActionDispatcher购买建筑(ActionDispatcher会进行验证，不需要在这里重复调用checkBlock)
				var action = {
					t: TD.getCurrentTick(),
					op: "place",
